...
机器人启动后会自动向 Telegram 注册快捷指令，包含 `/ban`、`/unban`、`/status`、`/help` 等管理命令。管理员私聊 `/start` 或在后台群使用 `/status`、`/help` 时，会看到快捷按钮：系统状态、封禁列表、管理指令说明。

## 安全验证流程

- 用户私聊 `/start` 后会看到欢迎信息和“开始安全验证”按钮。
- 验证题要求用户按照从小到大的顺序点击数字按钮。
- 用户可以点击“刷新验证题”更换题目。
